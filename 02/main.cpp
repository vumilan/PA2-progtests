...
unsigned CLandRegister::Count(const string &owner) const {
    ...
    if (!Owners.empty()) {
        ...
        // found owner in vector
        if (lower != upper) {
            return (*lower)->getOwnedCitiesVec().size();
        }
    }
    return 0;
}
    ...
    if (!VecCityAddr.empty()) {
        ...
        if (lower != upper) {
            ...
            if (!Owners.empty()) {
                ...
                if (lower_owner != upper_owner) {
                    delete ptrOwner;
                    ptrOwner = (*lower_owner);
                    return (*lower)->SelTo(ptrOwner, owner);
                }
            }
            // owner not found in vector, put new owner in vector of owners
            Owners.insert(lower_owner, ptrOwner);
            return (*lower)->SelTo(ptrOwner, owner);
        }
    }
    // if vector of cities empty or city is not found, return false
    delete ptrCity;
    return false;
}
...
bool CLandRegister::NewOwner(const string &region, unsigned int id, const string &owner) {
    ...
    if (!VecRegionId.empty()) {
        ...
        if (lower != upper) {
            ...
            // owner not found in vector, put new onwer in vector of owners
            Owners.insert(lower_owner, ptrOwner);
            return (*lower)->SelTo(ptrOwner, owner);
        }
    }
...
bool CLandRegister::Del(const string &city, const string &addr) {
    ...
    if (!VecCityAddr.empty()) {
        ...
        if (lower != upper) {
            delete ptrCity;
            ptrCity = (*lower);
            auto lower2 = lower_bound(VecRegionId.begin(), VecRegionId.end(), ptrCity, cityCityRegionIdCompare);
...
bool CLandRegister::Del(const string &region, unsigned int id) {
    ...
    if (!VecRegionId.empty()) {
        ...
        if (lower != upper) {
            delete ptrCity;
            ptrCity = (*lower);
            auto lower2 = lower_bound(VecCityAddr.begin(), VecCityAddr.end(), ptrCity, cityCityAddrCompare);
            // found city
